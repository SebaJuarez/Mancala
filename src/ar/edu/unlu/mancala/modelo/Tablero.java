...
// mover las habas segun la posicion indicada
	public Informe moverHabas(Posicion p , int jugadorActual) {	
		...
		if(habas== 0) return Informe.NOHAYHABAS;
		...
		//si la en la ultima posicion quedo una sola haba, entonces van a la casa del jugador todas las del lado contrario + esa misma
	...
	public boolean enRango(int jugador, Posicion pos) {
		return (jugador == 1)? pos.ordinal() >= Posicion.A.ordinal() && pos.ordinal() <= Posicion.F.ordinal() : pos.ordinal() >= Posicion.G.ordinal() && pos.ordinal() <= Posicion.L.ordinal();
	}
